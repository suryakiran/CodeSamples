...
int main (int argc, char** argv)
{
	...
	xpr::mark_tag numberTag(1), dateTag(1), monthTag(2), yearTag(3);
	pt::ptree tree ;

	xpr::sregex numberDesc = 
		as_xpr(tdAnchor) >> *_s >> skipAnchor >> *_s >> (numberTag= number) >> *_s >> skipAnchor >> *_s >> endTd >> *_s ;

	xpr::sregex dateReg =
		as_xpr(dateTag= xpr::repeat<1,2>(_d)) >> *_s >> (monthTag= longMonth) 
		>> *_s >> (yearTag= xpr::repeat<4,4>(_d))
		>> *_s >> skipAnchor >> *_s >> endTd >> *_s 
		>> xpr::repeat<10,10>(numberDesc)
		;
	...
	while (getline (fin, l, '\n'))
	{
		...
		if (xpr::regex_search (l, results, dateReg))
		{
			...
			for(; regIter != regIterEnd; ++regIter)
			{
				...
				string key = (
						format ("Bse Equity History.%1$02d-%2%-%3%")
